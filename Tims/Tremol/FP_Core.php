...
namespace ThemeFactory\Tims\Tremol {
    ...
    class EnumType
    {
        ...
        public function __construct($value)
        {
            if (self::isValidValue($value)) {
                ...
                throw new SException("Error while parsing Enum", ServerErrorTypeNew::ClientArgValueWrongFormat);
            }
        }
    ...
    class ServerErrorTypeNew extends EnumType
    {
        const OK = 0;
    ...
    class SException extends Exception
    {
        ...
        function __construct($message, $code, $previous = NULL, $in_ste1 = NULL, $in_ste2 = NULL)
        {
            ...
            if ($this->code == ServerErrorTypeNew::FPException) {
    ...
    class FP_Core
    {
        ...
        protected $ip = "127.0.0.1"; //localhost
        protected $port = 4444;
        protected $url = "http://127.0.0.1:4444/"; //http://localhost:4444/
        protected $w = FALSE;
        protected $ok = FALSE;
        ...
        private function sendReq($isPost, $u, $data)
        {
            try {
                ...
                throw new SException($msg, ServerErrorTypeNew::ServerConnectionError, $ex);
            }
        }
        ...
        private function throwOnServerError($resp)
        {
            ...
            try {
                ...
                throw new SException($ex->getMessage(), ServerErrorTypeNew::ClientXMLCanNotParse, $ex);
            }
        ...
        protected function execute($commandName, ...$args)
        {
            ...
            try {
                ...
                throw new SException($ex->getMessage(), ServerErrorTypeNew::ServerErr, $ex);
            } finally {
                $this->w = FALSE;
            }
        }
        ...
        public function ServerGetDeviceSettings()
        {
            ...
            try {
                ...
                $fps = array(
                    'IsWorkingOnTcp' => (bool)$settings->tcp,
        ...
        public function ServerGetClients()
        {
            ...
            try {
                ...
                foreach ($response->Tree->children() as $item) {
                    ...
                    $cl->is_connected = (bool)$item->PortIsOpen;
